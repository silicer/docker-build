...
def get_latest_version(project):
    ...
    if check_type == "github_release":
        ...
        latest_commit = subprocess.check_output(
            ["git", "-C", clone_path, "log", "-1", "--pretty=format:%H"]
        ).decode("utf-8")
...
def trigger_dispatch_event(project: dict, version):
    ...
    if isinstance(version, tuple):
        ...
        payload = {
            ...
            # if project.get("platforms", None) == None
            # else project["platforms"],
            # "repo": project["repo_url"],
            "version": version,
            # "latest": "true"
            # if project.get("latest", None) == None
            # else str(project["latest"]).lower(),
    ...
    if response.status_code == 204:
        ...
        print(
            f"Failed to dispatch event {event_type}: {response.status_code} {response.text}"
        )
